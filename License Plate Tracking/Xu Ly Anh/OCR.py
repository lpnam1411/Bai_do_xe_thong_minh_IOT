...
#Khởi tạo ID của thẻ từ ở đây sẽ dùng khối lệnh để lấy được giữ liệu qua I2C
...
c.execute('DROP TABLE IF EXISTS sqplite_sequence') 

c.execute('''Create TABLE IF NOT EXISTS users(id integer PRIMARY KEY AUTOINCREMENT, rfid TEXT,  username VACHAR, password VARCHAR, status BOOLEAN)''')
c.execute('''Create TABLE IF NOT EXISTS vehicles(id INTEGER PRIMARY KEY AUTOINCREMENT, numberplate TEXT,  status BOOLEAN)''')
c.execute('''Create TABLE IF NOT EXISTS parking(location VARCHAR, slots INTEGER, availble INTEGER, cost FLOAT, opentime TIME, closetime TIME)''')
...
while sensorIn:
    ...
    c.execute('INSERT INTO users(rfid, status) VALUES (?,?)', [RFID_IN,1])
    c.execute('INSERT INTO vehicles(numberplate, status) VALUES (?,?)', [second_string+third_string,1])
    conn.commit()
    print("Bien so xe la:")
    ...
    c.execute('select id from vehicles where (select rfid=? from users) and numberplate = ?',(RFID_OUT, numberPlateOut))
    output = c.fetchall()
    print(output)
    sensorIn = False;
    cv2.waitKey()
